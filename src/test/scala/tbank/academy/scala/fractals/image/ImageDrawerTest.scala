...
class ImageDrawerTest extends AnyFlatSpec with Matchers {
  it should "draw test" in {
    ...
    drawer.draw(imageData, outputStream) match {
      ...
      case None =>
        outputStream.close()
        outputStream.toByteArray.toList shouldBe List(
          -119, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 5, 0, 0, 0, 3, 8, 6, 0, 0, 0, 91, 54,
          -59, -8, 0, 0, 0, 21, 73, 68, 65, 84, 120, 94, 99, 96, 96, 96, -8, -113, 5, 35, 56, -1, -79, 9, -62, 48, 0,
          -35, 111, 15, -15, 7, -79, -55, -40, 0, 0, 0, 0, 73, 69, 78, 68, -82, 66, 96, -126
        )
    }
  }

//  Commented due to linter errors
//  it should "draw test#2" in {
//    val drawer       = new ImageDrawer()
